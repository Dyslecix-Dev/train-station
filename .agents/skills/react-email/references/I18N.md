      ...
      <Tailwind config={{ presets: [pixelBasedPreset] }}>
        ...
        <Body className="bg-gray-100 font-sans">
          <Container className="mx-auto max-w-xl px-5 py-10">
            ...
            <Text className="my-4 text-base leading-7 text-gray-800">{t("body")}</Text>
            <Button href={verificationUrl} className="block rounded bg-blue-600 px-5 py-3 text-center text-white no-underline">
              {t("cta")}
            </Button>
      ...
      <Tailwind config={{ presets: [pixelBasedPreset] }}>
        <Body className="bg-gray-100 font-sans">
          <Container className="mx-auto max-w-xl p-5">
            ...
            <Text className="text-base text-gray-800">{formatMessage({ id: "itemCount" }, { count: itemCount })}</Text>
...
export async function getT(namespace, locale) {
  ...
  return {
    t: i18next.getFixedT(locale ?? i18next.resolvedLanguage, Array.isArray(namespace) ? namespace[0] : namespace),
    i18n: i18next,
  };
}
```
...
import { Html, Body, Container, Heading, Text, Button, Tailwind, pixelBasedPreset } from "@react-email/components";
...
export default async function OrderConfirmation({ orderNumber, total, currency, locale, orderDate }: OrderConfirmationProps) {
  const t = createTranslator({
    messages: await import(`../messages/${locale}.json`),
      ...
      <Tailwind config={{ presets: [pixelBasedPreset] }}>
        ...
        <Body className="bg-gray-100 font-sans">
          <Container className="mx-auto max-w-xl px-5 py-10">
            ...
            <Text className="my-2 text-base text-gray-800">
              {t("order-number")}: {orderNumber}
            </Text>
            <Text className="my-2 text-base text-gray-800">
              {t("order-date")}: {dateFormatter.format(orderDate)}
            </Text>
            <Section className="my-4 rounded bg-white p-5">
              <Text className="text-xl font-bold text-gray-800">
                {t("total")}: {currencyFormatter.format(total)}
              </Text>
            </Section>
            <Button href={`https://example.com/orders/${orderNumber}`} className="my-5 block rounded bg-blue-600 px-5 py-3 text-center text-white no-underline">
              {t("view-order")}
            </Button>
            <Hr className="my-5 border-gray-200" />
            <Text className="text-sm text-gray-500">{t("footer")}</Text>
