...
// Usage
const emails = [{ from: "Acme <noreply@acme.com>", to: ["delivered@resend.dev"], subject: "Hello", html: "<p>Hi</p>" }];
...
async function sendBatchWithRetry(emails: Parameters<typeof resend.batch.send>[0], options: BatchSendOptions) {
...
class BatchEmailService {
  ...
  async send(emails: BatchEmail[], options: BatchSendOptions = {}): Promise<BatchResult> {
    const { idempotencyKey = `batch-${randomUUID()}`, maxRetries = 3, validateBeforeSend = true } = options;
  ...
  async sendLarge(
    ...
    const results = await Promise.all(chunks.map((chunk, index) => this.send(chunk, { idempotencyKey: `${batchPrefix}/chunk-${index}` }).then((result) => ({ chunkIndex: index, ...result }))));
